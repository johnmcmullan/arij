...
projects:
  - name: frontend
    ...
    # repo_url: git@tract-server:/opt/tract/git/frontend-tickets.git  ← used by `tract clone`

  - name: backend
    prefix: BACK
    path: ./backend
    description: API service
    # repo_url: git@tract-server:/opt/tract/git/backend-tickets.git

  - name: platform
    prefix: PLAT
    path: ./platform
    description: Cross-cutting platform work and documentation
    # depends_on: [FRONT, BACK]   ← tract clone PLAT also clones frontend + backend
...
**About `repo_url:` and `depends_on:`** — these fields are understood by `tract clone`. When a developer runs `tract clone PLAT`, the CLI fetches the catalog from the tract server (configured during install), resolves dependencies recursively, and clones each repo in order. `repo_url:` is the authoritative source for clone URLs; `path:` is the local relative path used in workspace tooling after cloning. Both fields are backward-compatible — existing code that doesn't know them simply ignores them.
