---
name: tract-schema
description: Create, read, update and query Tract tickets. Use when working with Jira tickets, markdown ticket files, tract CLI commands, logging time, or any ticket workflow. See workspace.md for this developer's repo locations.
---
# Tract Schema Skill
...
### Session Start: Pull Latest Tickets

**At the start of every session, pull to get the latest tickets synced from Jira:**

```bash
tract pull
```

This pulls all repos registered in `~/work/.tract/workspace.yaml` — project ticket repos and worklogs — so you always work with current data. Safe to run silently; it is a fast no-op when nothing has changed.

> If `tract pull` is not available (offline, no CLI), run `git pull` in each ticket repo manually.
...
**Mention → Link Rule (critical):** If your comment mentions another ticket ID (e.g. "see APP-1235 for context"), you MUST also add it to the `links` frontmatter in the same commit. Do both changes together:

```yaml
# In frontmatter — add this at the same time as the comment
links:
  - rel: related_to
    ref: APP-1235
```

Pick the right relationship type based on context:
- `related_to` — general context or discussion reference
- `blocked_by` — this ticket is waiting on that one
- `blocks` — this ticket is preventing that one
- `duplicate_of` — this is a duplicate

`tract doctor` will flag tickets with unlinked mentions, so gaps are detectable but prevention is better.
