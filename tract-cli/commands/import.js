...
async function importCommand(options) {
  ...
  // Import options
  const importOptions = {
    status: options.status || 'all',  // 'all' = no status filter (works for every project)
    limit: options.limit ? parseInt(options.limit) : null,
    jql: options.jql,
    resume: options.resume || false,
    concurrency: options.concurrency ? parseInt(options.concurrency) : null,
  ...
  try {
    ...
    if (isOneTimeImport) {
      ...
      console.log(chalk.gray('Tickets are in: tickets/\n'));
      ...
      console.log(chalk.gray('   git add tickets/'));
